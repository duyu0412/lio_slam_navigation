...
// 初始重力/姿态对齐矩阵，默认单位矩阵（如果需要重力对齐会被替换）
Eigen::Matrix3d rot_init = Eigen::Matrix3d::Identity();
bool init_baselink = false;
...
void publish_frame_body(const ros::Publisher &pubLaserCloudFull_body)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        pointBodyLidarToIMU(&feats_undistort->points[i],
                            &pt_imu);
        if (pt_imu.x > -2.0 && pt_imu.x < 2.0)
        {
            laserCloudIMUBody->push_back(pt_imu);
        }
    }
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
    ...
    // Use latest pose from graph optimization if available
    if (!keyframePoseQueue.empty())
    {
        gtsam::Pose3 latestPose = keyframePoseQueue.back();
        odomAftMapped.pose.pose.position.x = latestPose.x();
        odomAftMapped.pose.pose.position.y = latestPose.y();
        odomAftMapped.pose.pose.position.z = latestPose.z();
        odomAftMapped.pose.pose.orientation.x = latestPose.rotation().toQuaternion().x();
        odomAftMapped.pose.pose.orientation.y = latestPose.rotation().toQuaternion().y();
        odomAftMapped.pose.pose.orientation.z = latestPose.rotation().toQuaternion().z();
        odomAftMapped.pose.pose.orientation.w = latestPose.rotation().toQuaternion().w();
    }
    else
    {
...
void publish_opt_path(const ros::Publisher& pubOptPath){
    // Publish path from optimized keyframe poses
    nav_msgs::Path optimized_path;
    optimized_path.header.stamp = ros::Time().fromSec(lidar_end_time);
    optimized_path.header.frame_id = "odom";

    for (const auto& pose : keyframePoseQueue)
    {
        geometry_msgs::PoseStamped pose_stamped;
        pose_stamped.header.stamp = ros::Time().fromSec(lidar_end_time); // Note: timestamp is simplified
    ...
    pubOptPath.publish(optimized_path);
}
void publish_path(const ros::Publisher& pubPath)
{
    ...
    // 限制path长度，避免内存无限增长
    // 保留最近10000个位姿点（约10分钟@10Hz）
    if (path.poses.size() > 10000)
    {
        path.poses.erase(path.poses.begin(), path.poses.begin() + 1000);  // 删除最旧的1000个
    }
...
// 发布关键帧可视化信息
void publishKeyframeVisualization(
    const ros::Publisher& pubKeyframePath,
    const ros::Publisher& pubKeyframeCloud,
    const ros::Publisher& pubKeyframeMarkers)
{
    if (keyframePoseQueue.empty())
        return;
    ...
    keyframePath.header.stamp = ros::Time().fromSec(lidar_end_time);
    keyframePath.header.frame_id = "odom";

    for (size_t i = 0; i < keyframePoseQueue.size(); ++i)
    {
        const auto& pose = keyframePoseQueue[i];
        geometry_msgs::PoseStamped pose_stamped;
        ...
        pose_stamped.pose.orientation.z = pose.rotation().toQuaternion().z();
        pose_stamped.pose.orientation.w = pose.rotation().toQuaternion().w();
        keyframePath.poses.push_back(pose_stamped);
    }
    pubKeyframePath.publish(keyframePath);

    // 2. 发布关键帧位置标记 (球形标记)
    visualization_msgs::MarkerArray keyframeMarkers;
    for (size_t i = 0; i < keyframePoseQueue.size(); ++i)
    {
        const auto& pose = keyframePoseQueue[i];
        ...
        keyframeMarkers.markers.push_back(marker);
    }
    pubKeyframeMarkers.publish(keyframeMarkers);

    // 3. 发布所有关键帧的合并点云 (可选，每隔一定数量的关键帧发布一次以节省性能)
    static int publish_cloud_count = 0;
    publish_cloud_count++;
    if (publish_cloud_count % 10 == 0)  // 每10个关键帧发布一次合并点云
    {
        PointCloudXYZI::Ptr mergedCloud(new PointCloudXYZI());
        
        // 只合并最近的一些关键帧点云，避免内存过大
        int start_idx = std::max(0, (int)keyframeCloudQueue.size() - 50);
        for (size_t i = start_idx; i < keyframeCloudQueue.size(); ++i)
        {
            if (keyframeCloudQueue[i] != nullptr)
            {
                *mergedCloud += *keyframeCloudQueue[i];
            }
        }
        ...
        if (!mergedCloud->empty())
        {
            ...
            downSizeFilter.setInputCloud(mergedCloud);
            PointCloudXYZI::Ptr mergedCloudDS(new PointCloudXYZI());
            downSizeFilter.filter(*mergedCloudDS);

            sensor_msgs::PointCloud2 cloudMsg;
            pcl::toROSMsg(*mergedCloudDS, cloudMsg);
            cloudMsg.header.stamp = ros::Time().fromSec(lidar_end_time);
            cloudMsg.header.frame_id = "odom";
            pubKeyframeCloud.publish(cloudMsg);
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    parameters.relinearizeThreshold = isam2_relinearize_threshold;
    parameters.relinearizeSkip = isam2_relinearize_skip;
    isam = new gtsam::ISAM2(parameters);

    // Initialize noise models using parameters
    priorNoise = gtsam::noiseModel::Diagonal::Variances(
        (gtsam::Vector(6) << prior_noise_rotation, prior_noise_rotation, prior_noise_rotation, 
                             prior_noise_translation, prior_noise_translation, prior_noise_translation).finished());
    odometryNoise = gtsam::noiseModel::Diagonal::Variances(
        (gtsam::Vector(6) << odometry_noise_rotation, odometry_noise_rotation, odometry_noise_rotation, 
                             odometry_noise_translation, odometry_noise_translation, odometry_noise_translation).finished());
    loopNoise = gtsam::noiseModel::Diagonal::Variances(
        (gtsam::Vector(6) << loop_noise_rotation, loop_noise_rotation, loop_noise_rotation, 
                             loop_noise_translation, loop_noise_translation, loop_noise_translation).finished());

    printf("[GTSAM] ISAM2 initialized: relinearizeThreshold=%.3f, relinearizeSkip=%d\n", 
           isam2_relinearize_threshold, isam2_relinearize_skip);
    printf("[GTSAM] Prior noise: rot=%.1e, trans=%.1e\n", prior_noise_rotation, prior_noise_translation);
    printf("[GTSAM] Odometry noise: rot=%.1e, trans=%.1e\n", odometry_noise_rotation, odometry_noise_translation);
    printf("[GTSAM] Loop noise: rot=%.1e, trans=%.1e\n", loop_noise_rotation, loop_noise_translation);
    ...
    ros::Publisher pubOptPath = nh.advertise<nav_msgs::Path>("/pointlio/opt_path", 100000);

    ros::Publisher plane_pub = nh.advertise<visualization_msgs::Marker>("/pointlio/planner_normal", 1000);

    // Backend optimization visualization publishers
    ros::Publisher pubKeyframePath = nh.advertise<nav_msgs::Path>("/pointlio/keyframe_path", 100);
    ros::Publisher pubKeyframeCloud = nh.advertise<sensor_msgs::PointCloud2>("/pointlio/keyframe_cloud", 100);
    ros::Publisher pubKeyframeMarkers = nh.advertise<visualization_msgs::MarkerArray>("/pointlio/keyframe_markers", 100);
    ros::Publisher pubLoopClosureMarkers = nh.advertise<visualization_msgs::MarkerArray>("/pointlio/loop_closure_markers", 100);
    pubLoopClosureMarkersGlobal = pubLoopClosureMarkers;  // 设置全局发布器
    // Global publishers for loop closure visualization
    ...
    while (ros::ok())
    {
        ...
        // Add keyframe, add factors and check for loop closure
        addKeyframeAndDetectLoop();

        // Publish keyframe visualization
        publishKeyframeVisualization(pubKeyframePath, pubKeyframeCloud, pubKeyframeMarkers);
    ...
    if (pcd_save_en)
    {
        std::cout << "Saving optimized map..." << std::endl;
        
        // 使用优化后的位姿和 body frame 点云生成地图
        PointCloudXYZI::Ptr optimizedMap(new PointCloudXYZI());
        
        for (size_t i = 0; i < keyframeCloudBodyQueue.size(); ++i)
        {
            if (keyframeCloudBodyQueue[i] == nullptr || i >= keyframePoseQueue.size())
                continue;
            
            // 用优化后的位姿将 body frame 点云变换到世界坐标系
            gtsam::Pose3 optimizedPose = keyframePoseQueue[i];
            Eigen::Matrix4f transformMatrix = Eigen::Matrix4f::Identity();
            transformMatrix.block<3,3>(0,0) = optimizedPose.rotation().matrix().cast<float>();
            transformMatrix.block<3,1>(0,3) = optimizedPose.translation().cast<float>();
            
            PointCloudXYZI::Ptr transformedCloud(new PointCloudXYZI());
            pcl::transformPointCloud(*keyframeCloudBodyQueue[i], *transformedCloud, transformMatrix);
            
            *optimizedMap += *transformedCloud;
        }
        
        // 根据参数决定是否下采样
        if (!optimizedMap->empty())
        {
            PointCloudXYZI::Ptr mapToSave;
            
            if (map_save_downsample_en)
            {
                pcl::VoxelGrid<PointType> downSizeFilterSave;
                downSizeFilterSave.setLeafSize(map_save_resolution, map_save_resolution, map_save_resolution);
                downSizeFilterSave.setInputCloud(optimizedMap);
                PointCloudXYZI::Ptr optimizedMapDS(new PointCloudXYZI());
                downSizeFilterSave.filter(*optimizedMapDS);
                mapToSave = optimizedMapDS;
                std::cout << "Downsampling enabled, resolution: " << map_save_resolution << "m" << std::endl;
            }
            else
            {
                mapToSave = optimizedMap;
                std::cout << "Downsampling disabled, saving full resolution map" << std::endl;
            }
            
            string file_name = string("optimized_map.pcd");
            string all_points_dir(string(string(ROOT_DIR) + "PCD/") + file_name);
            std::cout << "Saving optimized map to: " << all_points_dir << std::endl;
            std::cout << "Optimized map size: " << mapToSave->size() << " points" << std::endl;
            pcl::PCDWriter pcd_writer;
            pcd_writer.writeBinary(all_points_dir, *mapToSave);
        }
        ...
        if (pcl_wait_save->size() > 0)
        {
            ...
            string all_points_dir(string(string(ROOT_DIR) + "PCD/") + file_name);
            std::cout << "Saving raw accumulated map to: " << all_points_dir << std::endl;
            pcl::PCDWriter pcd_writer;
            pcd_writer.writeBinary(all_points_dir, *pcl_wait_save);
        }
    }
